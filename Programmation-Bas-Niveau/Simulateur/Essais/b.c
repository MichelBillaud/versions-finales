...
*/
struct my_tentry * 
make_my_tentry(char *k,char *name)
{
    struct my_tentry *mt = 
...
int mt_compare_func(const void *l, const void *r)
{
    ...
    return 0;
    */
    return strcmp(ml->mt_key, mr->mt_key); 
}
...
walk_entry(const void *mt_data,VISIT x,int level)
{
    ...
    printf("<%d>Walk on node %s key=%s name=%s  \n",
        level,
        x == preorder?"preorder":
    ...
    twalk(tree1,walk_entry);
    {
        ...
        if (r) {
            ...
            if(r) {
               ...
               printf("tdelete returned parent: %s %s\n",
